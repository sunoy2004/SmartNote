"""
Supabase Setup and Model Upload Script
Creates the required bucket and uploads trained models and tokenizer to Supabase Storage
"""
import os
import sys
import torch
...
def setup_and_upload():
    """Create bucket and upload trained models to Supabase"""
    try:
        ...
        print("Checking if 'models' bucket exists...")
        try:
            ...
            if "models" not in bucket_names:
                ...
                supabase.storage.create_bucket("models", {
                    "public": True,
                    "file_size_limit": 52428800,  # 50MB
                    "allowed_mime_types": ["application/octet-stream", "application/json"]
                })
                print("✓ 'models' bucket created successfully")
            else:
        ...
        print("\nSetup and upload process completed!")
        
    except Exception as e:
        print(f"Error during setup and upload: {e}")
        raise
